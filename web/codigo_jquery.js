$(document).on('ready', function() {
    ...
    $(document).on('click', '#datos_criterios', function() {
        ...
        if(id_fichero == "")
            ...
            var url;
            if(alpha != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+alpha;
            else
                url = document.location.href+"/api/"+test+"/"+id_fichero;
    ...
    $(document).on('click', '#datos', function() {
        ...
        if(id_fichero == "")
            ...
            if(alpha != "no" && tipo != "no" && test_post_hoc != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+alpha+"/"+tipo+"/"+test_post_hoc;
            else if(test_post_hoc != "no" && alpha != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+alpha+"/"+test_post_hoc;
            else if(test_post_hoc != "no" && tipo != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+tipo+"/"+test_post_hoc;
            else if(test_post_hoc != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+test_post_hoc;
            else if(alpha != "no" & tipo !="no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+alpha+"/"+tipo;
            else if(alpha != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+alpha;
            else if(tipo != "no")
                url = document.location.href+"/api/"+test+"/"+id_fichero+"/"+tipo;
            else
                url = document.location.href+"/api/"+test+"/"+id_fichero;
    ...
    $(document).on('click', '#boton_fichero', function() {
        ...
        if(!fichero)
            ...
            $.ajax({
                type: "POST",
                url: document.location.href+"/api/fichero",
                dataType: "json",
                data: formData,
                cache: false,
                contentType: false,
    ...
    $(document).on('click', '#boton_consulta', function() {
        ...
        if( $("#consultar_hashmd5").val()!="") {
            ...
            $.ajax({
                type: "GET",
                url: document.location.href+"/api/fichero/"+consulta_id,
                success : function(data) {
